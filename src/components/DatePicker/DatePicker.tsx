...
const DatePicker: React.FC<DatePickerProps> = ({
  ...
  const isDateSelected = (day: number, isCurrentMonth: boolean, isNextMonth: boolean) => {
    if (!selected) return false;
    let checkDate: Date;
    if (!isCurrentMonth && !isNextMonth) {
      checkDate = new Date(viewDate.getFullYear(), viewDate.getMonth() - 1, day);
    } else if (isNextMonth) {
      checkDate = new Date(viewDate.getFullYear(), viewDate.getMonth() + 1, day);
    } else {
  ...
  return (
    <div className={`datepicker ${className}`} ref={containerRef}>
      <div className="datepicker__input" onClick={openCalendar}>
        <span className={selected ? 'datepicker__value' : 'datepicker__placeholder'}>
            ...
            <div className="datepicker__days">
              {days.map((day, index) => (
                <button
                  key={index}
                  type="button"
                  onClick={() => handleDateClick(day.date, day.isCurrentMonth, day.isNextMonth)}
